            ...
            if isinstance(typeToConsume, protoTypes.RepeatedValueType):
                ...
                while offset < typeToConsume.minRepeat:
                    if not self.consumeNextValue(vtype, values):
                        ...
                        return self.failed(
                            "expected repeated value type %r" % typeToConsume
                        )
                    offset += 1
                while (
                    typeToConsume.maxRepeat is None or offset < typeToConsume.maxRepeat
                ):
                    if not self.consumeNextValue(vtype, values):
                        break
                    offset += 1
                ...
                for vtype in typeToConsume.vtypes:
                    if not self.consumeNextValue(vtype, values):
                        values[:] = self.originalValues
                        return self.failed(
                            "expected compound value type %r" % typeToConsume
                        )
...
class Key(Consumer):
    ...
    def create(self, *values):
        ...
        if not self.consume(keyword):
            raise KeysError(
                "value types do not match for keyword %s: %r" % (self.name, values)
            )
        return keyword

    def describeAsHTML(self):
        content = utilHtml.Div(
            utilHtml.Div(self.name, className="keyname"), className="key"
        )
    ...
    def describe(self):
        ...
        if self.help:
            ...
            formatted = textwrap.fill(
                textwrap.dedent(self.help).strip(), width=66
            ).replace("\n", pad)
    ...
    def load(dictname, forceReload=False):
        ...
        try:
            ...
            if not dictname == kdict.name:
                raise KeysDictionaryError(
                    "dictionary filename and name "
                    "are different: %s, %s" % (modulespec.name, kdict.name)
                )
